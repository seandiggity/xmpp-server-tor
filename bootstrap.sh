...
mkdir /usr/share/lua/5.1/mod_otr
cp $PWD/usr/lib/prosody/modules/* /usr/lib/prosody/modules/
cp $PWD/usr/lib/prosody/modules/mod_otr.lua /usr/share/lua/5.1/mod_otr/
...
echo ""
echo "3. Edit /etc/prosody/prosody.cfg.lua so that all cases of 'example.org' are replaced with your domain."
echo ""
echo "4. After you reboot, you can run 'aa-enforce /etc/apparmor.d/usr.bin.prosody' to update apparmor rules."
echo ""
echo "5. After the reboot, be sure to run 'cat /var/lib/tor/xmpp/hostname' to get your Tor hidden service address."
echo ""
echo "6. The option to allow SSH as an authenticated Tor hidden service has been supplied. Just read the comments in /etc/tor/torrc."
echo ""
